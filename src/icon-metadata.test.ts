...
describe("web icon metadata", () => {
  it("connects browser, Apple, Safari, and PWA assets", () => {
    ...
    expect(
      document.querySelector('link[rel="mask-icon"][href="/safari-pinned-tab.svg"]')?.getAttribute("color"),
    ).toBe("#0369A1");
    expect(document.querySelector('link[rel="manifest"][href="/site.webmanifest"]')).not.toBeNull();
    expect(document.querySelector('meta[name="theme-color"]')?.getAttribute("content")).toBe("#0369A1");
